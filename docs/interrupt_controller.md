...
A basic interrupt controller is embedded in the mainboard CPLD. Given the limited resources of the CPLD the interrupt controller is designed to be a no frills device. To support ELKS with only minimal changes to the code the interrupt controller mimics some of the features of an Intel 8259A.
...
There are two I/O ports for communicating with the interrupt controller
 - 0x18 Command
 - 0x1A EOI (end of interrupt)

At power up all four IRQ's are disabled.

The IRQ's can be controlled by writing to the command port (1 == enable, 0 == disable):
 - Bit 7-4 ignored
 - Bit 3 IRQ3
 - Bit 2 IRQ2
 - Bit 1 IRQ1
...
Reading from the command port returns the current status of the IRQ's (enabled/disabled).

When an IRQ is disabled the hardware interrupt line is ignored and any previously unacknowledged interrupts are lost.

When an IRQ is enabled it must see a positive edge on the hardware line before it will raise an interrupt with the CPU. Any further positive edges are ignored until the interrupt has been acknowledged.
...
An EOI can be sent by writing any value to the EOI port. 
 ...
 - Add additional IRQ lines using the USER1 -> USER4 connections on the bus.
